...
bool Character::operator==(const Character& Maryash) const        //same name, same race, same level and are either both an enemy or not.
{
    if(name_ == Maryash.name_ && race_ == Maryash.race_ && level_ == Maryash.level_ && enemy_ == Maryash.isEnemy())
    {
        return true;
    }
    return false;
...
bool Character::operator!=(const Character& Maryash) const
{
    if(name_ != Maryash.name_ || race_ != Maryash.race_ || level_ != Maryash.level_ || enemy_ != Maryash.isEnemy())
    {
        return true;
...
void Character::display() const
{
    ...
    if(enemy_ == true)
    {
        ligorio = "They are an enemy";
    }
    else
    {
        ligorio = "They are not an enemy";
    }
    std::cout << name_ << " is a Level " << level_ << " " << race_ << "." << "\nVitality: " << vitality_ << "\n Max Armor: " << armor_ << "\n" << ligorio << "\n";
}