...
//September 26, 2023
//ArrayBag.cpp defines the constructors and private and public function implementation of the Character class
...
template <class ItemType>
void ArrayBag<ItemType>::operator/=(const ArrayBag<ItemType>& noDupes) 
{
   for(int i = 0; i < getCurrentSize(); i++)
   {
      ...
      if(!contains(noDupes.items_[i]))
      {
         add(noDupes.items_[i]);
      }
   }
}

template <class ItemType>
void ArrayBag<ItemType>::operator+=(const ArrayBag<ItemType>& yesDupes)
{
   for(int i = 0; i < getCurrentSize(); i++)
   {
      if(getCurrentSize() == DEFAULT_CAPACITY)
