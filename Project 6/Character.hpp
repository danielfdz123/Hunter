...
//enum AcTion with the values {BUFF_Heal,BUFF_MendMetal,ATT_Strike,ATT_ThrowTomato}
enum Action 
{
...
{
    std::string name_;
    int turns_;           //the number of turns this Buff effect will last for
};
...
//Helper function and other stuff
        void addActiontoQueue(std::string name, int turns);
        bool emptyActionQueue();
        void popActions();
        
        std::queue<int> getActionQueue();
        void pushAction(int action);
        std::stack<Buff> getBuffs();
