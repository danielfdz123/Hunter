...
//HELPER FUNCTIONS
void Character::addActiontoQueue(std::string name, int turns)
{
    //If buff is HEAL or MENDMETAL, then its given its corresponding name, and turns, and added to the queue
...
bool Character::emptyActionQueue()
{
    //Simple enough... as long as the action queue isnt empty, every action within it is popped (deleted)
...
void Character::popActions()
{
    action_queue_.pop();            //Pops a single actions from the queue when called
}

std::queue<int> Character::getActionQueue()
{
  return action_queue_;             //Returns action queue when called
}

void Character::pushAction(int action)
{
    action_queue_.push(action);     //Pushes action into the queue when called
}

std::stack<Buff> Character::getBuffs()
{
  return buff_stack_;               //Returns buff stack when called
}