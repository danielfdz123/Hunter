/*
CSCI235 Fall 2023
Project 4: Algorithm Adventures - Into the Recursive Realms
...
void Ranger::display()const
{
    ...
    std::cout << "Animal Companion: " << (has_companion_ ? "TRUE" : "FALSE") << std::endl;
...
void Ranger::eatTaintedStew()
{
    ...
    if(getRace() != "UNDEAD")
    {
        ...
        {
            if(getAffinities()[i] == "POISON")
            {
                setVitality(std::round(getVitality()/2));
                badGuy = true;                                        //possible else statement instead of having a whole new if statement shit?????
            }
        }
    }
    if(getRace() != "UNDEAD" && badGuy == false)
    {
        setVitality(1);
    }
    if(getRace() != "UNDEAD" && getCompanion() == true)
    {
